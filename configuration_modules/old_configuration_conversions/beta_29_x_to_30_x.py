...
from configuration_modules.old_configuration_conversions.config_reset import reset_primary_config, \
    reset_installed_sensors, reset_variance_config
...
def upgrade_beta_29_to_30():
    ...
    try:
        ...
        new_primary_config.enable_display = int(primary_config_lines[1])
        new_primary_config.enable_interval_recording = int(primary_config_lines[2])
        new_primary_config.enable_trigger_recording = int(primary_config_lines[3])
        new_primary_config.sleep_duration_interval = float(primary_config_lines[4])
        new_primary_config.enable_custom_temp = int(primary_config_lines[5])
        new_primary_config.temperature_offset = float(primary_config_lines[6])
        new_primary_config.web_portal_port = int(primary_config_lines[7])
