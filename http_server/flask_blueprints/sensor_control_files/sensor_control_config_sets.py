...
html_sensor_control_receive_routes = Blueprint("html_sensor_control_receive_routes", __name__)


@html_sensor_control_receive_routes.route("/ReceivePushConfiguration", methods=["POST"])
@auth.login_required
def receive_push_config():
...
def receive_online_services():
    logger.network_logger.info("* Received 'Active Online Services' from " + str(request.remote_addr))
    ...
    if request.form.get("online_service_selected_action") == "weather_underground":
    ...
    elif request.form.get("online_service_selected_action") == "luftdaten":
    ...
    elif request.form.get("online_service_selected_action") == "open_sense_map":
        if send_interval < 10.0:
            send_interval = 10.0
